...
//         try {
//             setLoading(true);
//             const response = await axios.get('http://localhost:9595/api/v1/notesdb');
...
function Notescomp() {
    ...
    const fetchNotes = useCallback(async () => {
        try {
            ...
            setDeleteStatus({ message: 'Error fetching notes. Please try again later.', type: 'error' });
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleDelete = async (notesId) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:9595/api/v1/notesdb/delete/${notesId}`);
            setDeleteStatus({ message: 'Note deleted successfully', type: 'success' });
            // Update the notes state immediately after successful deletion
            setNotes(prevNotes => prevNotes.filter(note => note._id !== notesId));
        } catch (err) {
            console.error("Error deleting note:", err);
            setDeleteStatus({ message: 'Error deleting note. Please try again later.', type: 'error' });
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <table className="lg:w-full md:w-auto text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        {notesDetails.map((note, index) => (
                            <tr key={note._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td className="px-6 py-4 w-2/6">{note.description}</td>
